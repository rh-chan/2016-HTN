...
# list_of_candidates = ['Mcds', 'Arbys', 'Wendys', 'Busan', 'Ben Tanh']
# data = [{'Mcds': 2, 'Arbys': 2, 'Wendys': 0, 'Busan': 1, 'Ben Tanh': 0},
#         {'Mcds': 1, 'Arbys': 0, 'Wendys': 2, 'Busan': 2, 'Ben Tanh': 1},
#         {'Mcds': 1, 'Arbys': 1, 'Wendys': 1, 'Busan': 0, 'Ben Tanh': 2},
#         {'Mcds': 2, 'Arbys': 1, 'Wendys': 0, 'Busan': 1, 'Ben Tanh': 1},
#         {'Mcds': 2, 'Arbys': 1, 'Wendys': 0, 'Busan': 0, 'Ben Tanh': 1}]
#
...
def parse_ranks(ranks):
    ...
    for single_rank in ranks:
        ...
        #converting dictionary format to list format [[a, b], [c], [d, e]]
        for key, value in sorted(single_rank.iteritems(), reverse=False):
            v[value].append(key)

        single_rank_list = [value for key, value in sorted(v.iteritems(), reverse=True)]

        single_rank_key = str(single_rank_list)
        if single_rank_key in parsed_ranks_dict:
            parsed_ranks_dict[single_rank_key] += 1
        else:
            parsed_ranks_dict[single_rank_key] = 1

    parsed_rank_sequence = [(ast.literal_eval(key), value) for key, value in parsed_ranks_dict.iteritems()]
    return parsed_rank_sequence
...
# output = parse_ranks(data)
#
# ranked_candidates = compute_ranks(list_of_candidates, output)
